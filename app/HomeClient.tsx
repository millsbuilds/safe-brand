...
function Nav({ isHSN }: { isHSN: boolean }) {
    ...
    <header style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, fontFamily: F.base }}>
      ...
      >
        <p style={{ margin: 0, fontSize: 11, fontWeight: 400, color: C.peach, letterSpacing: "0.06em", fontFamily: F.base }}>
          {isHSN
            ? "A physician-founded public benefit corporation"
...
function Hero({ isHSN }: { isHSN: boolean }) {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        >
          {isHSN ? "Health Science Nutritionals, PBC" : "SafeBrand™"}
        </p>
        ...
        >
          {isHSN
            ? "Physician-formulated nutrition. Clinical-grade precision."
        ...
        >
          {isHSN
            ? "Health Science Nutritionals is a science-first public benefit corporation that develops clinical-grade mineral nutrition products — formulated by a physician, built for patients, practitioners, and health systems."
...
function Footer({ isHSN }: { isHSN: boolean }) {
  const [isMobile, setIsMobile] = useState(false);
  useEffect(() => {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        >
          <div style={{ fontSize: isHSN ? 18 : 22, fontWeight: 700, color: C.white, letterSpacing: "-0.02em" }}>
            {isHSN ? "Health Science Nutritionals, PBC" : <>SafeBrand<span style={{ color: C.peach }}>™</span></>}
          </div>
...
export default function HomeClient({ isHSN }: { isHSN: boolean }) {
  return (
    <>
      <Nav isHSN={isHSN} />
      <main>
